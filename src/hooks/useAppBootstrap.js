 ...
 * 
 * 使用项目真实基础设施：
 * - 请求：src/api/base.js (dispatch)
 * - 用户状态：src/store/modules/user.js
 * - 路由常量：src/config/routes.js
 * - 错误对象：src/config/errors.js
 * - 启动配置：src/config/startup.js
 * 
 * 注意：品牌最短展示时长已移至 src/config/startup.js 统一管理
 * 本 hook 只负责初始化逻辑，不负责 UI 时长控制
 * 
 * 启动阶段所有请求使用 authStrategy: 'passive'，
 * 401 直接 reject，由调用页面决定怎么处理
 */

import { getBootstrapContext, checkAppVersion } from '../api/auth.js'
import { useUserStore } from '../store/modules/user.js'
import storage from '../utils/storage.js'
import { LOGIN, APPLY_SIGN, ACCOUNT_STATUS, HOME } from '../config/routes.js'
import { AppError, ErrorKind } from '../config/errors.js'
