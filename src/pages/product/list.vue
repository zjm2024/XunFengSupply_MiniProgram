<template>
  <view class="page">
    ...
    <scroll-view class="page-scroll" scroll-y :style="{ height: scrollHeight }" @scrolltolower="loadMore">
      <view class="content">
        <!-- 搜索框 -->
...
import { safeNavigateTo, safeNavigateBack } from '@/utils/routeGuard.js'
...
// 购物车数量
const cartCount = computed(() => cartStore.summary.skuCount)

// 计算是否还有更多
const canLoadMore = computed(() => hasMore.value && !loading.value)

onMounted(async () => {
  // 页面高度交给 flex + AppHeader 自身状态栏适配处理，
  // 避免不同手机 / iPad 因固定导航栏高度估算造成底部留白或裁切。
...
function goToCart() {
  safeNavigateTo('/pages/cart/index')
}

function goToDetail(item) {
  if (item && item.productId) {
    safeNavigateTo(`/pages/product/detail?id=${item.productId}`)
  }
}
</script>
