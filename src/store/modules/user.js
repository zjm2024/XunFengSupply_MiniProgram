...
import { getCreditLevel } from '../../utils/business.js'
...
export const useUserStore = defineStore('user', {
  state: () => ({
    ...
    signStatus: 0,                 // 签约状态 0未签约 1审核中 2已签约 -1拒绝
  ...
  actions: {
    ...
    async logout() {
      ...
      try {
        const { useOrderStore } = await import('./order.js')
        const orderStore = useOrderStore()
        orderStore.clearPendingPaymentOrder()
        orderStore.clearCurrentOrderDetail()
        orderStore.setOrderCounts({ pendingReview: 0, pendingPayment: 0, shipped: 0, afterSale: 0, all: 0 })
      } catch (e) {
        // 忽略
      }
