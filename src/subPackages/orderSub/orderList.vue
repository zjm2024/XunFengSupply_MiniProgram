      ...
      <AppContent>
        <view class="order-list-page">
          ...
          <scroll-view class="list-wrapper" scroll-y @scrolltolower="loadMore">
            ...
            <view class="order-list" v-if="!loading || orderList.length > 0">
              ...
              >
                ...
                <view class="card-header">
                  ...
                  <status-tag :statusValue="order.orderStatus" type="order" />
                </view>
                ...
                <view class="goods-preview">
                  <image
                    class="goods-img"
                    :src="order.firstItemImageUrl || '/static/images/jersey-red.png'"
                    mode="aspectFill"
                  />
            ...
            <empty-view v-if="!loading && orderList.length === 0" text="暂无订单" />
...
import { getOrderList, getOrderCounts, cancelOrder, confirmReceipt, generateClientRequestId } from '../../api/order.js'
...
function goToDetail(orderId) {
  safeNavigateTo(`/subPackages/orderSub/orderDetail?orderId=${orderId}`)
}
...
async function handleAction(key, order) {
  switch (key) {
    case 'pay':
      safeNavigateTo(`/subPackages/paySub/payPage?orderId=${order.orderId}`)
      break
    case 'cancel':
      uni.showModal({
        title: '提示',
      ...
      break
    case 'logistics':
      safeNavigateTo(`/subPackages/orderSub/orderDetail?orderId=${order.orderId}`)
      break
  }
}
</script>
