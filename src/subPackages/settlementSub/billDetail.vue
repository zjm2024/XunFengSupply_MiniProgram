      ...
      <AppContent>
        <view class="bill-detail-page" v-if="billInfo.billId">
          ...
          <view class="bill-header">
            ...
            <status-tag :statusValue="billInfo.status" type="bill" />
          </view>
    ...
    <template #footer>
      <FixedActionBar v-if="billInfo.billId">
        <button class="export-btn" @click="handleExport">
          <uni-icons type="download" size="16" color="#C41E3A" />
          导出PDF凭证
        </button>
        <button 
          class="pay-btn" 
 ...
 */
function handlePay() {
  safeNavigateTo(`/subPackages/paySub/payPage?billId=${billId.value}&payMode=1&amount=${billInfo.value.amount - billInfo.value.paidAmount}`)
}
</script>
