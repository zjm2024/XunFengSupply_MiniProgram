      ...
      <AppContent>
        <view class="message-list-page">
          ...
          <scroll-view class="list-wrapper" scroll-y @scrolltolower="loadMore">
            ...
            <empty-view v-if="!loading && messageList.length === 0" text="暂无消息" />
...
/**
 * 加载消息列表
 * TODO: 调用 getMessageList({ type: currentType.value })
 */
async function loadMessages() {
  loading.value = true
  try {
    // const res = await getMessageList({ type: currentType.value })
...
/**
 * 加载未读数量
 * TODO: 调用 getUnreadCount() 接口
 */
async function loadUnreadCount() {
  try {
    // const res = await getUnreadCount()
    // messageStore.setUnreadCount(res)
  } catch (e) {}
}
...
async function goToDetail(item) {
  ...
  // 根据消息类型跳转不同页面
  const routes = {
    1: `/subPackages/orderSub/orderDetail?orderId=${item.relatedId}`,
    2: `/subPackages/paySub/payPage?orderId=${item.relatedId}`,
    3: `/subPackages/orderSub/orderDetail?orderId=${item.relatedId}`,
    4: `/subPackages/afterSaleSub/afterSaleList`,
    5: `/subPackages/settlementSub/billList`,
    99: null
  }
