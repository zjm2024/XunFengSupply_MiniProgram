      ...
      <AppContent>
        <view class="after-sale-list-page">
          ...
          <scroll-view class="list-wrapper" scroll-y @scrolltolower="loadMore">
            <view class="as-list">
              ...
              >
                ...
                <view class="card-header">
                  ...
                  <status-tag :statusValue="item.status" type="afterSale" />
                </view>
            ...
            <empty-view v-if="!loading && asList.length === 0" text="暂无售后记录" />
...
import { getAfterSaleList, cancelAfterSale } from '../../api/aftersale.js'
...
/**
 * 加载售后列表
 * TODO: 调用 getAfterSaleList({ status: currentStatus.value })
 */
async function loadAsList() {
  loading.value = true
  try {
    // const res = await getAfterSaleList({ status: currentStatus.value })
